...
interface AddServidorModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAddSuccess: () => void;
}

const DEFAULT_CATEGORIA_ID = 3;
...
const createDefaultForm = (): ServidorForm => {
...
export const AddServidorModal = ({
  isOpen,
  onClose,
  onAddSuccess,
}: AddServidorModalProps) => {
  const [formData, setFormData] = useState<ServidorForm>(createDefaultForm);
  ...
  const handleChange = (
    ...
    const numericFields = new Set(["ubicacion_id", "valor_compra"]);
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (
      ...
      toast.error(
        "Completa los campos obligatorios para registrar el servidor.",
      );
      return;
    }
    ...
    try {
      ...
      toast.error("No se pudo registrar el servidor. Intenta nuevamente.");
    } finally {
          ...
          <div className="flex items-start justify-between gap-4 border-b border-slate-200/80 bg-slate-50/80 px-6 py-5 sm:px-8">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm text-slate-500">
                Registra un nuevo servidor enviando el body esperado por el
                endpoint de activos.
              </p>
            </div>
            <button
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  >
                    ...
                    {ubicaciones.map((ubicacion) => (
                      <option key={ubicacion.id} value={ubicacion.id}>
                        {ubicacion.nombre_lab ??
                          ubicacion.nombre ??
                          `Ubicación ${ubicacion.id}`}
                        {ubicacion.edificio_salon
                          ? ` - ${ubicacion.edificio_salon}`
                          : ""}
                      </option>
                    ))}
              ...
              <div className="flex items-center justify-end gap-3 border-t border-slate-200 pt-5">
                ...
                <button
                  ...
                  disabled={isSubmitting || isLoadingUbicaciones}
                >
